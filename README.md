...
2. **Deployment** - Event grid subscriptions require the endpoint to be live. In order to get the application deployed, there must be a 2 step deployment process. Comment out the event grid subscription creation in the AppHost project, deploy the application, then uncomment the event grid subscription creation and deploy the application again. See https://github.com/Azure/azure-dev/issues/3931 for more details.